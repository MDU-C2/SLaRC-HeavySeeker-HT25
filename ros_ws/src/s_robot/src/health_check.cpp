...
class HealthCheckNode : public rclcpp::Node {
    ...
    HealthCheckNode()
     : Node("health_check") {
        ...
        // declare ros params
        this->declare_parameter<std::string>("platform.motor", "/motor_ok");
        ...
        // publishers
        pub_saftey_stop = this->create_publisher<std_msgs::msg::Bool>("/safety_stop", 10);
        pub_zero_vel = this->create_publisher<geometry_msgs::msg::TwistStamped>("/emergency/cmd_vel", 10);
    }
    ...
    std_msgs::msg::Bool lock_telop_control, lock_local_control, lock_auto_control;
    std_msgs::msg::Bool saftey_stop;
    geometry_msgs::msg::TwistStamped zero_vel;
    rclcpp::Publisher<geometry_msgs::msg::TwistStamped>::SharedPtr pub_zero_vel;
    ...
    void callback_sub_manual_controllers(sensor_msgs::msg::Joy joy) {
        ...
        // only save time recived, the actual input is not used
        this->status_controller.time_recived = this->get_clock()->now();
    }

    void activate_safety_lock() {
        // send 0 velocity
        this->zero_vel.header.stamp = this->get_clock()->now();
        pub_zero_vel->publish(this->zero_vel);
