...
#include "s_robot/motor_interface.hpp"
  ...
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  for (const auto & [name, ID]: motorCanID) {
    frame.can_id = get_eid(CMD_SET_MODE, ID) | CAN_EFF_FLAG;
...
hardware_interface::return_type motor_interface ::DiffBotSystemHardware::write(
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  ...
  for (const auto & [name, descr] : joint_command_interfaces_) {
    ...
    RCLCPP_INFO(get_logger(), "CAN 0x%X → %.3f rad/s → %d", frame.can_id, get_command(name), eRPM);
  }
...
inline int32_t DiffBotSystemHardware::radToErpm(double rads) {
  ...
  return static_cast<int32_t>(rads * (60 / (2*M_PI)) * POLE_PAIRS * MOTOR_REDUCTION_RATIO);
}
...
PLUGINLIB_EXPORT_CLASS(
  motor_interface::DiffBotSystemHardware, hardware_interface::SystemInterface)
